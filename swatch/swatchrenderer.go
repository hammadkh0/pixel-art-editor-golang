...
type SwatchRenderer struct {
	...
	parent  *Swatch //references swatch that created this renderer
}
...
func (renderer *SwatchRenderer) Layout(size fyne.Size) {
	// amount of space we have to draw our swatch
	renderer.objects[0].Resize(size) // this will resize existing square to the available size provided
}
