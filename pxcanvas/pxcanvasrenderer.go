...
type PxCanvasRenderer struct {
	...
	canvasBorder []canvas.Line // Lines are used to draw lines where ever we want in a canvas. Use lines slice to create rectangle border
}

// WidgetRenderer interface implementations below
...
func (renderer *PxCanvasRenderer) Objects() []fyne.CanvasObject {
	...
	for i := 0; i < len(renderer.canvasBorder); i++ {
		// append the border lines to objects that fyne will use to create borders
